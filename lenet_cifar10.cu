...
int main()
{
    printf("Starting LeNet-5 CIFAR-10 Training on GPU (Fixed Conv Gradients)...\n");
    ...
    int conv1_w_size = 6 * 3 * 5 * 5;  
    int conv1_out_size = 6 * 28 * 28;  
    int p1_out_size = 6 * 14 * 14;     
    int conv2_w_size = 16 * 6 * 5 * 5; 
    int conv2_out_size = 16 * 10 * 10; 
    int p2_out_size = 16 * 5 * 5;      
    int fc1_w_size = 120 * 400;        
    int fc2_w_size = 84 * 120;         
    int fc3_w_size = 10 * 84;          
    ...
    init_weights_xavier(h_w_fc1, fc1_w_size, 400, FC1_SIZE);
    ...
    for (int epoch = 0; epoch < EPOCHS; epoch++)
    {
        for (int file_idx = 0; file_idx < NUM_TRAIN_BATCHES; file_idx++)
        {
            ...
            for (int batch = 0; batch < num_batches; batch++)
            {
                ...
                if (current_batch_size > BATCH_SIZE) current_batch_size = BATCH_SIZE;
                ...
                fc_forward_kernel<<<fc1_grid, fc1_block>>>(
                    ...
                    current_batch_size, 400, FC1_OUT, true);
                cudaCheck(cudaDeviceSynchronize());
                ...
                fc_weight_grad_kernel<<<grad_fc1_grid, grad_fc1_block>>>(
                    ...
                    current_batch_size, 400, FC1_OUT);
                cudaCheck(cudaDeviceSynchronize());
    ...
    printf("==========================================\n");
    printf("Training complete in %.2f ms\n", train_milliseconds);
    printf("==========================================\n");

    printf("Loading test batch and running inference...\n");
    ...
    for (int batch = 0; batch < test_batches; batch++)
    {
        ...
        dim3 fc1_grid((FC1_OUT + fc1_block.x - 1) / fc1_block.x, current_batch_size);
        fc_forward_kernel<<<fc1_grid, fc1_block>>>(
            d_p2_out, d_w_fc1, d_b_fc1, d_fc1_out, current_batch_size, 400, FC1_OUT, true);
    ...
    cudaFree(d_w_c1); cudaFree(d_b_c1);
    cudaFree(d_w_c2); cudaFree(d_b_c2);
    cudaFree(d_w_fc1); cudaFree(d_b_fc1);
    cudaFree(d_w_fc2); cudaFree(d_b_fc2);
    cudaFree(d_w_fc3); cudaFree(d_b_fc3);
    
    cudaFree(d_conv1_out); cudaFree(d_p1_out);
    cudaFree(d_conv2_out); cudaFree(d_p2_out);
    cudaFree(d_fc1_out); cudaFree(d_fc2_out); cudaFree(d_fc3_out);
    
    cudaFree(d_probs); cudaFree(d_loss);
    
    cudaFree(d_d_fc3); cudaFree(d_d_fc2); cudaFree(d_d_fc1);
    cudaFree(d_d_p2); cudaFree(d_d_conv2); cudaFree(d_d_p1); cudaFree(d_d_conv1);
    
    cudaFree(d_d_w_c1); cudaFree(d_d_b_c1);
    cudaFree(d_d_w_c2); cudaFree(d_d_b_c2);
    cudaFree(d_d_w_fc1); cudaFree(d_d_b_fc1);
    cudaFree(d_d_w_fc2); cudaFree(d_d_b_fc2);
    cudaFree(d_d_w_fc3); cudaFree(d_d_b_fc3);
    ...
    free(h_w_c1); free(h_b_c1);
    free(h_w_c2); free(h_b_c2);
    free(h_w_fc1); free(h_b_fc1);
    free(h_w_fc2); free(h_b_fc2);
    free(h_w_fc3); free(h_b_fc3);
